...
if ~isempty(window2) % if only one averaging window is needed, put an empty window2
    ...
    avg_values = zeros(N_periods, 1); % Store average values for each period
    values = signal.signals.values;
    t=signal.time;
    for k = 1:N_periods
        % Define time limits for this period
    ...
    % Display results
    disp('Averages for each period:');
    disp(array2table(avg_values, 'VariableNames', {'Window'}));
